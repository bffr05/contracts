...
contract OperatorableClient is TrustableClient, IOperatorable {
    ...
    function supportsInterface(
        bytes4 interfaceId_
    ) public view virtual override(TrustableClient) returns (bool) {
        return
    ...
    function setOperator(address operator_, bool approved_) public virtual {
        if (isReferred())
            if (ITrustable(referral()).isTrusted(address(this)))
                return
                    IROperatorable(referral()).setOperator(
                        _msgSender(),
                        operator_,
                        approved_
                    );
    ...
    modifier onlyOperator(address user_) {
        if (!isOperator(user_, _msgSender()))
    ...
    function getOperators(
        ...
        //OK
        if (isReferred()) return IOperatorable(referral()).getOperators(user_);
...
contract OperatorableServer is
    TrustableClient,
    OperatorableClient,
    TrustableServer,
    IROperatorable
{
    ...
    function isTrusted(
        ...
        view
        virtual
        override(TrustableClient, TrustableServer)
        returns (bool)
    {
        //OK
