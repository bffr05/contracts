...
async function makebalance(from,to,balance)
{
    ...
    console.log(`totransfer = ${ethers.utils.parseEther(totransfer.toString())}`)
    if (totransfer <=0) 
        return;
        ...
        await tresp.wait(1)
    console.log(`balance = ${await to.getBalance()}`)
...
async function deployer() {
    ...
    console.log(`mainuser=${signer.address} balance=${await signer.getBalance()} nonce=${await signer.getTransactionCount()}`)
