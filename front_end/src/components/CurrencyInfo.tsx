...
export const CurrencyInfo = ({ address, type, symbol, name, decimals, image }: CurrencyInfoProps) => {
    //console.log("address=",address," type=",type," symbol=",symbol," name=",name, " decimals=",decimals, " image=",image);
    ...
    const [amount, setAmount] = useState(formatBalance(stackBalance).toString());
    useEffect(() => {
        setAmount(formatBalance(stackBalance).toString());
      }, [stackBalance]);
    
    //const [current, setCurrent] = useState<String>(address);
